...
bool buf_cmp(const void *_buf1, const void *_buf2, unsigned size)
{
	...
	const uint8_t *buf1 = (uint8_t*)_buf1, *buf2 = (uint8_t*)_buf2;
	...
	const void *_mask, unsigned size)
{
	...
	const uint8_t *buf1 = (uint8_t*)_buf1, *buf2 = (uint8_t*)_buf2, *mask = (uint8_t*)_mask;
	unsigned last = size / 8;
	...
	void *_buf, unsigned buf_len, unsigned radix)
{
	...
	uint8_t *b256_buf = (uint8_t*)calloc(b256_len, 1);
...
void bit_copy_queue_init(bit_copy_queue *q)
{
	bit_copy_queue_lock.lock();

	q->clear();

	bit_copy_queue_lock.unlock();
}


int bit_copy_queued(bit_copy_queue *q, uint8_t *dst, unsigned int dst_offset, uint8_t *src, unsigned int src_offset, unsigned int bit_count)
{
	bit_copy_queue_entry qe = {};	
...
void bit_copy_execute(bit_copy_queue *q)
{
	bit_copy_queue_lock.lock();
	
	for (bit_copy_queue::iterator it = q->begin(); it != q->end(); ++it)
	{
		bit_copy_queue_entry qe = *it;
...
void bit_copy_discard(bit_copy_queue *q)
{
	bit_copy_queue_lock.lock();
