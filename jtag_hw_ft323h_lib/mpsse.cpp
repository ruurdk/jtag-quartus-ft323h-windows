...
#define LOG_ERROR DebugLog
#define LOG_WARNING DebugLog
#define LOG_DEBUG DebugLog
...
{
	struct mpsse_ctx* ctx = (mpsse_ctx*)calloc(1, sizeof(*ctx));
	int err;

	if (!ctx)
		return 0;

	ctx->read_queue = &read_queue;
	bit_copy_queue_init(ctx->read_queue);
	ctx->read_chunk_size = 16384;
	ctx->read_size = 16384;
	ctx->write_size = 16384;
	ctx->read_chunk = (uint8_t*)malloc(ctx->read_chunk_size);
	ctx->read_buffer = (uint8_t*)malloc(ctx->read_size);
	ctx->write_buffer = (uint8_t*)malloc(ctx->write_size);
	...
	if (!open_matching_device(ctx, vid, pid, description, serial, location)) {
		...
		LOG_ERROR("unable to open ftdi device with vid %s, pid %s, description '%s', "
			...
			vid ? sprintf_s(vidstr, "%04x", *vid), vidstr : "*",
			pid ? sprintf_s(pidstr, "%04x", *pid), pidstr : "*",
...
{
	DEBUG_IO("buffer_write_byte %02x", data);
...
static unsigned buffer_add_read(mpsse_ctx* ctx, uint8_t* in, unsigned in_offset,
	unsigned bit_count, unsigned offset)
{
	DEBUG_IO("buffer_add_read (queued) - %d bits, offset %d", bit_count, offset);
	assert(ctx->read_count + DIV_ROUND_UP(bit_count, 8) <= ctx->read_size);
	bit_copy_queued(ctx->read_queue, in, in_offset, ctx->read_buffer + ctx->read_count, offset, bit_count);
...
{
	DEBUG_IO("mpsse_clock_tms_cs - %sout %d bits, tdi=%d", in ? "in" : "", length, tdi);
	assert(out);

	if (ctx->retval != ERROR_OK) {
		LOG_DEBUG("Ignoring command due to previous error");
		return;
	}
...
{
	struct transfer_result* res = (transfer_result*)transfer->user_data;
	...
	DEBUG_IO("raw chunk %d, transferred %d of %d", transfer->actual_length, res->transferred, ctx->read_count);
...
int mpsse_flush(struct mpsse_ctx* ctx)
{
	...
	if (retval != ERROR_OK) {
		LOG_DEBUG("Ignoring flush due to previous error");
		assert(ctx->write_count == 0 && ctx->read_count == 0);
		ctx->retval = ERROR_OK;
		return retval;
	}

	//LOG_DEBUG("mpsse_flush - write %d%s, read %d", ctx->write_count, ctx->read_count ? "+1" : "",	ctx->read_count);
	...
	struct transfer_result read_result = { ctx, /* done */ true, 0 };
	if (ctx->read_count) {
	...
	struct transfer_result write_result = { ctx, /*done */ false, 0 };
